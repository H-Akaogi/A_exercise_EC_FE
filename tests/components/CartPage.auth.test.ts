...
vi.mock("@/components/hooks/usePaymentMethod", () => ({
  usePaymentMethod: () => ({
    paymentMethods: [
      {
        id: 1,
    ...
    isLoading: false,
    errorMessage: "",
    findAll: mocks.findAllPaymentMethods,
  }),
}));
...
describe("CartPage 顧客認証", () => {
  beforeEach(() => {
    mocks.isAuthenticated = false;
    mocks.isInitialized = true;
    mocks.replace.mockReset();
    mocks.findAllPaymentMethods.mockReset().mockResolvedValue(undefined);
  ...
  it("未ログインで購入確定を選ぶとログイン画面へ遷移する", async () => {
