...
describe("ProductRepository", () => {
  ...
  describe("findByCategory", () => {
    ...
    it("APIエラーのmessageを例外として投げる", async () => {
      fetchMock.mockResolvedValue({
        ...
        status: 500,
        json: vi.fn().mockResolvedValue({
          message: "商品を取得できませんでした。",
        }),
      });

      await expect(repository.findByCategory()).rejects.toThrow(
        "商品を取得できませんでした。",
      );
    });
  ...
  describe("findById", () => {
    it("商品UUIDを指定して商品詳細を取得できる", async () => {
      ...
      fetchMock.mockResolvedValue({
        ok: true,
        status: 200,
        json: vi.fn().mockResolvedValue(productDetail),
      });
    ...
    it("404の場合はnullを返す", async () => {
      ...
      const result = await repository.findById("not-found-product");
  ...
  describe("purchase", () => {
    ...
    it("購入を確定できる", async () => {
      ...
      await expect(repository.purchase(1, validItems)).resolves.toBeUndefined();
    ...
    it.each([
      ...
      async ({ paymentMethodId }) => {
        await expect(
          repository.purchase(paymentMethodId, validItems),
        ).rejects.toThrow("支払い方法を選択してください。");

        expect(fetchMock).not.toHaveBeenCalled();

        expect(customerAuthServiceMock.getAccessToken).not.toHaveBeenCalled();
      },
    );

    it("購入商品が空の場合は例外を投げる", async () => {
      await expect(repository.purchase(1, [])).rejects.toThrow(
        "購入する商品がありません。",
      );
    ...
    ])("購入商品の$title場合は例外を投げる", async ({ items }) => {
      await expect(repository.purchase(1, items)).rejects.toThrow(
        "購入商品の内容が不正です。",
      );
    ...
    it("アクセストークンがない場合は認証情報を削除して例外を投げる", async () => {
      ...
      await expect(repository.purchase(1, validItems)).rejects.toThrow(
        "購入するにはログインが必要です。",
      );
    ...
    it("APIエラーのerrorsを結合して例外として投げる", async () => {
      ...
      fetchMock.mockResolvedValue({
        ...
        text: vi.fn().mockResolvedValue(
          JSON.stringify({
            errors: {
              Items: ["購入商品が不正です。", "在庫数を確認してください。"],
              PaymentMethod: "支払い方法が不正です。",
            },
          }),
        ),
      });
    ...
    it("JSONでないレスポンス本文をそのまま例外として投げる", async () => {
      ...
      fetchMock.mockResolvedValue({
        ok: false,
        status: 500,
        text: vi.fn().mockResolvedValue("サーバーとの通信に失敗しました。"),
      });
