...
describe("OrderRepository 顧客JWT認証", () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.unstubAllGlobals();
  });

  it("購入履歴一覧へBearer JWTを付ける", async () => {
    const { service, getAccessToken } = createAuthService("customer-jwt");
    const repository = new OrderRepository(service);
  ...
  it("購入履歴詳細へBearer JWTを付ける", async () => {
    const { service } = createAuthService("customer-jwt");
    const repository = new OrderRepository(service);
    const fetchMock = vi.fn().mockResolvedValue(
      createJsonResponse(200, {
  ...
  it("一覧APIの401で保存済み認証情報を削除する", async () => {
    const { service, clearAuthentication } = createAuthService("expired-jwt");
    const repository = new OrderRepository(service);

    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(createJsonResponse(401, {})),
    );
  ...
  it("詳細APIの401で保存済み認証情報を削除する", async () => {
    const { service, clearAuthentication } = createAuthService("expired-jwt");
    const repository = new OrderRepository(service);
    ...
    await expect(repository.findById("order-uuid")).rejects.toThrow(
