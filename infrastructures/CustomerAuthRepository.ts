 ...
 */
@injectable()
export class CustomerAuthRepository implements ICustomerAuthRepository {
  public async login(
    ...
    const responseData =
      (await response.json()) as Partial<CustomerLoginResponse>;

    if (
      typeof responseData.accessToken !== "string" ||
      responseData.accessToken.length === 0 ||
      typeof responseData.expiresAt !== "string" ||
      !Number.isFinite(Date.parse(responseData.expiresAt))
    ) {
      throw new Error("ログイン応答の形式が正しくありません。");
    }

    const username = normalizeOptionalUsername(responseData.username);
  ...
  public async logout(accessToken: string): Promise<CustomerLogoutResponse> {
    let response: Response;
    ...
    const responseData =
      (await response.json()) as Partial<CustomerLogoutResponse>;

    if (responseData.loggedOut !== true) {
      throw new Error("ログアウト応答の形式が正しくありません。");
    }
...
const readErrorMessage = async (
  ...
  return (
    errorData.message ?? errorData.detail ?? errorData.title ?? fallbackMessage
  );
};

const normalizeOptionalUsername = (value: unknown): string | undefined => {
