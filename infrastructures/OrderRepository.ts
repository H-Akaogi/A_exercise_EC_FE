...
export class OrderRepository implements IOrderRepository {
  ...
  public async findPurchaseHistory(): Promise<SearchOrdersResponse> {
    ...
    if (response.status === 401) {
      ...
      throw new Error("購入履歴を確認するにはログインが必要です");
    }
    ...
    return {
      title: "購入履歴",
      orderList: responseData.orderList.map((order) => ({
  ...
  public async findById(orderUuid: string): Promise<Orders> {
    ...
    if (response.status === 401) {
      ...
      throw new Error("購入履歴の詳細を確認するにはログインが必要です");
    }
    ...
    if (response.status === 404) {
      throw new Error("指定された購入履歴が見つかりませんでした");
    }
    ...
    if (!response.ok) {
      ...
      console.error("========== PURCHASE HISTORY DETAIL API ERROR ==========");
      console.error("url:", url);
      console.error("status:", response.status);
      console.error("error body:", errorData);
      console.error("=======================================================");
     ...
     */
    return {
      orderUuid: responseData.orderUuid,

      orderDate: responseData.orderDate,

      amountTotal: responseData.totalPrice,

      orderStatus: {
        id: responseData.orderStatusId,

        name: responseData.orderStatusName,
      },

      ordersDetails: responseData.orderItems.map((item, index) => ({
        /*
         * APIから注文明細IDは返らないため、
         * 画面表示用の連番を設定する。
         */
