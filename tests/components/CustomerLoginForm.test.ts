...
const mocks = vi.hoisted(() => ({
  login: vi.fn(),
  sessionMessage: null as string | null,
  replace: vi.fn(),
}));
...
vi.mock("@/components/hooks/useCustomerAuth", () => ({
  useCustomerAuth: () => ({
    ...
    expiresAt: null,
    isInitialized: true,
    sessionMessage: mocks.sessionMessage,
    login: mocks.login,
    logout: vi.fn(),
...
describe("CustomerLoginForm", () => {
  ...
  it("セッションタイムアウトの案内を表示する", () => {
    mocks.sessionMessage = "セッションが切れました。再度ログインしてください";

    render(createElement(CustomerLoginForm));
  ...
  it("メールアドレスとマスクされたパスワード入力を表示する", () => {
    ...
    expect(passwordInput.closest("form")).toHaveAttribute("method", "post");
  });

  it("認証成功後にトップ画面へ遷移する", async () => {
    ...
    await user.type(screen.getByLabelText("パスワード"), "Test12345");
    await user.click(
  ...
  it("401の汎用エラーを表示し入力値を保持する", async () => {
    mocks.login.mockRejectedValue(
    ...
    const mailInput = screen.getByLabelText("メールアドレス");
    const passwordInput = screen.getByLabelText("パスワード");

    await user.type(mailInput, "ando.taro@example.com");
    ...
    expect(mailInput).toHaveValue("ando.taro@example.com");
