...
test.describe(
    ...
    () => {
        ...
        const targetCategoryName =
            process.env
                .E2E_PRODUCT_CATEGORY_NAME
            ?? "食品";

        const targetProductName =
            process.env
                .E2E_CATEGORY_PRODUCT_NAME
            ?? "カリン　のど飴";

        const otherCategoryProductName =
            process.env
                .E2E_OTHER_CATEGORY_PRODUCT_NAME
            ?? "製図用シャープペンシル";
        ...
        test(
            "商品カテゴリを選択すると、そのカテゴリに属する商品のみ表示される",
            async ({ page }) => {
                /**
                 * 商品カテゴリの選択欄を取得する。
                 *
                 ...
                 */
                const targetProductExists =
                    await findProductAcrossPages(
                        page,
                        targetProductName,
                    );
        ...
        test(
            "カテゴリを変更すると、変更後のカテゴリの商品一覧へ更新される",
            async ({ page }) => {
                const categorySelect =
                    page.getByLabel(
                        "商品カテゴリ",
